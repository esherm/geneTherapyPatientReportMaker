...
gene_names_barplot <- unique(barplotAbunds$maskedRefGeneName)
...
n_timpoints_for_cell_type <- barplotAbunds %>%
    distinct(CellType, Timepoint) %>%
    group_by(CellType) %>%
    summarize(TimePointCount = n())

n_timpoints_for_cell_type <- mutate(
    n_timpoints_for_cell_type, 
    FractionTimePointCount = TimePointCount / max(TimePointCount)
)

barplotAbunds <- left_join(barplotAbunds, n_timpoints_for_cell_type, by='CellType') 

ggplot(data=barplotAbunds, aes(Timepoint, estAbundProp, fill=maskedRefGeneName)) +
  geom_bar(aes(width=0.9*FractionTimePointCount), stat="identity") + facet_wrap(~CellType, scales="free") + scale_fill_manual(values=siteColors) +
