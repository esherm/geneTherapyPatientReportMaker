...
if( nrow(sites.multi) < 1 ) { cat("None found") } else { 
...
if( !any(is.multi.expanded) ) { cat("None found") } else {
	df <- subset(sites.multi, is.multi.expanded)
    df <- df %>% group_by(Patient, Timepoint, CellType) %>% mutate(Rank=estAbund)
	df <- data.frame(Timepoint=df$Timepoint,
		CellType=df$CellType,
		GTSP=df$GTSP,
		SonicAbundance=df$estAbund,
		Fraction=df$estAbund/total.abundance,
		multihitID=df$multihitID,
		Rank=df$Rank
	)			
