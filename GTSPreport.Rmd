...
In the barplots below, any sites with Estimated Absolute Abundance below `r abundCutoff.total.barplots` are binned as LowAbund.

```{r totalBarPlots, fig.height=12, fig.width=12}
gene_names_barplot <- unique(c(barplotTotalAbunds$maskedRefGeneName, barplotSampleAbunds$maskedRefGeneName))
...
n_timepoints_for_cell_type <- barplotTotalAbunds %>%
    distinct(CellType, Timepoint) %>%
    group_by(CellType) %>%
    summarize(TimePointCount = n())

n_timepoints_for_cell_type <- mutate(
    n_timpoints_for_cell_type, 
    FractionTimePointCount = TimePointCount / max(TimePointCount)
)

barplotTotalAbunds <- left_join(barplotTotalAbunds, n_timepoints_for_cell_type, by='CellType') 

ggplot(data=barplotTotalAbunds, aes(Timepoint, estAbund, fill=maskedRefGeneName)) +
...
<P style="page-break-before: always">
Below are similar barplots to the previous figure, yet show the relative abundance of the top 10 clones in each sample.  Cutoff values for binning clones as LowAbund are calulated based on the Estimated Absolute Abundance from each sample and are noted next to the cell types.

```{r sampleBarPlots, fig.height=12, fig.width=12}
n_timepoints_for_cell_type <- barplotSampleAbunds %>%
    distinct(CellType, Timepoint) %>%
    group_by(CellType) %>%
    summarize(TimePointCount = n())
...
barplotSampleAbunds <- left_join(barplotSampleAbunds, n_timepoints_for_cell_type, by='CellType') 
barplotSampleAbunds$CellType <- paste0(
  as.character(barplotSampleAbunds$CellType),
  " (cutoff = ",
  abundCutoff.sample.barplots[barplotSampleAbunds$CellType],
  " cells)")
barplotSampleAbunds <- split(barplotSampleAbunds, barplotSampleAbunds$CellType)

#Relative abundance by CellTypes
do.call(
  grid.arrange,
  lapply(barplotSampleAbunds, function(data){
...
```{r sitetype_heatmap, fig.width=12, fig.height=16}
ggplot(data=detailedAbunds, aes(Timepoint, maskedRefGeneName, fill=estAbundProp)) + geom_tile() +
  ...
  facet_wrap(~CellType, scales="free_x", ncol=6) + labs(y="SiteType", x="Timepoint") +
  theme(axis.text.x=element_text(angle=45, hjust=1, vjust=1))
```
