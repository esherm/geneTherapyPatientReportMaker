...
## Do any clones account for greater than `r percent(percent_cutoff)` of the total?
...
```{r wordlelist, fig.width=5, fig.height=5, fig.show='hide'}
...
dftmp <- df %>%
    ...
    select(word=nearest_refSeq_gene, freq=estAbund) 
df.wordlist <- dlply(dftmp, .(Timepoint, CellType))
max_num_words=100
...
df.tab$html <- paste0("<img src='",df.tab$pngfile,"' alt='wordBubble'>")
df.tab$isthere <- file.exists(df.tab$pngfile)
if(any(!df.tab$isthere)) message("The word bubbles are not generated for the following time point and cell types:\n", paste(df.tab$name[!df.tab$isthere], collapse="\n") )
df.tab <- subset(df.tab, isthere)
df.kable <- matrix(df.tab$html, ncol=2)
