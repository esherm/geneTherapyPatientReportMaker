...
if( any(rows) ) {
	...
	df <- subset(df, select=c("seqnames", "strand", "position", "GTSP", "estAbund", "estAbundProp", "estAbundRank", "Timepoint", "CellType", "nearest_refSeq_geneDist", "nearest_refSeq_gene", "nearest_refSeq_geneOrt", "X5pNrstOncoDist", "X5pNrstOnco", "X5pNrstOncoOrt"))
	...
	df$nearest_gene=paste0(df$nearest_refSeq_gene, " (", df$nearest_refSeq_geneOrt, ")")
	df$nearest_Gene_Strand_Distance=paste0(df$nearest_refSeq_gene, " (", df$nearest_refSeq_geneOrt, ") ", df$nearest_refSeq_geneDist)
	df$nearest_OncoGene_Strand_Distance=paste0(df$X5pNrstOnco, " (", df$X5pNrstOncoOrt, ") ", df$X5pNrstOncoDist)
	needed <- c("Chr", "strand", "position", "GTSP", "SonicAbundance", "Fraction", "Rank", "Timepoint",	"CellType", "nearest_Gene_Strand_Distance", "nearest_OncoGene_Strand_Distance")
...
##tmp <- sites.multi %>% group_by(Patient, Timepoint, CellType) %>%
##	mutate(istop=(Rank==min(Rank)))
##is.multi.expanded <- tmp$istop
...
Each integration site is named by the closest gene.
...
dups <- names(which(table(df.kable)>1))
if(length(dups)>0) {
  dup_pos <- sapply(1:length(dups), function(i){
    which(df.kable == dups[i])})[2,]
  df.kable[dup_pos] <- ""
}
kable(df.kable, row.names=FALSE,format="html", escape=FALSE)
```
